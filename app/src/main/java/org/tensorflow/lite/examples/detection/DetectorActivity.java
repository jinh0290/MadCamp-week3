...
public class DetectorActivity extends CameraActivity implements OnImageAvailableListener {
  ...
  private static final String TF_OD_API_MODEL_FILE2 = "mask_detector.tflite";
    ...
    trackingOverlay.addCallback(
            new DrawCallback() {
              ...
              public void drawCallback(final Canvas canvas) {
                ...
                tracker2.draw(canvas);
                if (isDebug()) {
                  tracker2.drawDebug(canvas);
                }
              }
            });
  ...
  protected void processImage() {
    ...
    faceDetector2
            ...
            .addOnSuccessListener(new OnSuccessListener<List<Face>>() {
              ...
              public void onSuccess(List<Face> faces) {
                if (faces.size() == 0) {
                  updateResults2(currTimestamp, new LinkedList<>());
                  return;
                }
                runInBackground(
                        new Runnable() {
                          @Override
                          public void run() {
                            onFacesDetected2(currTimestamp, faces);
                          }
                        });
              }
  ...
  private void onFacesDetected(long currTimestamp, List<Face> faces, boolean add) {
    ...
    for (Face face : faces) {
      ...
      if (boundingBox != null && goodConfidence) {
        ...
        cvFace2.drawBitmap(portraitBmp, matrix, null);
...
//    }
  ...
  private void onFacesDetected2(long currTimestamp, List<Face> faces) {
...
//      results = detector.recognizeImage(croppedBitmap);
      ...
      if (boundingBox != null && goodConfidence) {
        ...
        // translates portrait to origin and scales to fit input inference size
        //cv.drawRect(faceBB, paint);
        float sx = ((float) TF_OD_API_INPUT_SIZE2) / faceBB.width();
        float sy = ((float) TF_OD_API_INPUT_SIZE2) / faceBB.height();
        Matrix matrix = new Matrix();
        matrix.postTranslate(-faceBB.left, -faceBB.top);
        matrix.postScale(sx, sy);
        ...
        if (resultsAux2.size() > 0) {

          Classifier.Recognition result2 = resultsAux2.get(0);

          float conf = result2.getConfidence();
          if (conf >= 0.6f) {
    ...
    updateResults2(currTimestamp, mappedRecognitions);
